...
export default function StickyHeroSection() {
  ...
  useGSAP(() => {
    // Gunakan matchMedia agar ScrollTrigger HANYA aktif di layar Desktop (min-width: 1024px)
    // Di Mobile, GSAP tidak akan berjalan, sehingga sangat ringan
  ...
  return (
    <>
      ...
      {/* MOBILE HERO (SWIPER) */}
      {/* ======================= */}
      <section className="block lg:hidden relative w-full h-[100dvh] bg-slate-900">
        <Swiper
          ...
          loop={true}
          className="w-full h-full sticky-hero-swiper"
        >
          {sections.map((sec, i) => (
            <SwiperSlide key={sec.id}>
              <div className={`relative w-full h-full ${sec.bgColor} overflow-hidden`}>
                ...
                <div className="absolute inset-0 h-[65%]">
                  <Image
                    ...
                    priority={i === 0}
                  />
                  {/* Gradients to blend smoothly into content */}
                  <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/30 to-black/40" />
                </div>
                
                {/* Text Content Area */}
                <div className="absolute inset-x-0 bottom-0 h-[50%] p-6 flex flex-col justify-end pb-24 z-10 bg-gradient-to-t from-slate-900 via-slate-900 to-transparent">
                  <span className="text-amber-400 font-bold tracking-[0.2em] uppercase text-xs block mb-3 drop-shadow-md">
                    {sec.subtitle}
                  </span>
                  <h2 className="text-4xl font-black text-white mb-4 leading-[1.1] drop-shadow-lg">
                    {sec.title}
                  </h2>
                  <p className="text-slate-300 text-sm mb-8 leading-relaxed drop-shadow-md opacity-90 max-w-sm">
                    {sec.description}
                  </p>
                  <button className="group w-fit flex items-center gap-3 px-6 py-3 bg-white text-slate-900 font-bold rounded-full hover:bg-amber-400 hover:text-blue-900 transition-all duration-300 text-sm shadow-lg shadow-white/10">
                    JELAJAHI
        ...
        <style dangerouslySetInnerHTML={{ __html: `
          .sticky-hero-swiper .swiper-pagination-bullet {
          ...
          .sticky-hero-swiper .swiper-pagination-bullet-active {
            background: #fbbf24;
            width: 24px;
            border-radius: 12px;
          }
      ...
      {/* DESKTOP HERO (GSAP) */}
      {/* ======================= */}
      <section ref={containerRef} className="hidden lg:block relative w-full h-screen bg-slate-900 overflow-hidden">
        {/* Sidebar Navigation */}
        <div className="absolute left-10 top-1/2 -translate-y-1/2 z-50 flex flex-col gap-8">
          {sections.map((sec) => (
          ...
          {sections.map((sec, i) => (
            ...
            >
              ...
              <div className="relative z-10 w-full md:w-1/2 p-8 md:p-20 flex flex-col justify-center h-full">
                ...
                <button className="group w-fit flex items-center gap-4 px-8 py-4 md:px-10 md:py-5 bg-white text-slate-900 font-bold rounded-full hover:bg-amber-400 hover:text-blue-900 transition-all duration-300 text-sm md:text-base shadow-lg shadow-white/10">
                  JELAJAHI
